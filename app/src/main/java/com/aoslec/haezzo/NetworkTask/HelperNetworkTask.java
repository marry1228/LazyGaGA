...
public class HelperNetworkTask extends AsyncTask<Integer,String,Object> {
    ...
    ArrayList<HelperListBean> helperListBeans;//---> 아 bean 만들어야 겠다

    // Network Task를 검색 입력 수정 삭제 구분없이 하나로 사용키 위해 생성자 변수 추가.
    String where = null;

    //Constructor
    public HelperNetworkTask(Context context, String mAddr, String where) {
        //누가 부른다, 주소는 뭐야 , 입력인지 수정인지 삭제인지
        //예 ) insertActivity가 불럿어, 주소는 뭐고 insert 할거야
        this.context = context;
        this.mAddr = mAddr;
        this.helperListBeans = new ArrayList<HelperListBean>();//입력할때도 어레이 쓰고 그래야 해서
    ...
    protected Object doInBackground(Integer... integers) {
        ...
        //
        if(where.equals("select")){
            return helperListBeans; // select는 엄청 많은 값이 들어올거임
        }else{
    ...
    private void parserSelect(String str){
        ...
        try{
            ...
            JSONArray jsonArray = new JSONArray(jsonObject.getString("helper_info"));
            Log.v("Message","jsonArray 진입");
            helperListBeans.clear();
            ...
            for(int i=0; i<jsonArray.length(); i++){
                ...
                Log.v("Message","unumber:" + unumber + "uimage :" + uimage);
                //어레이에 있는거 뽑아와서 빈
                HelperListBean helperListBean = new HelperListBean(unumber,uimage,uage,ufm,unickname,uaddress,hgaga,hrating);
                helperListBeans.add(helperListBean);
